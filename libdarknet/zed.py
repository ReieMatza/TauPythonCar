...
def cvDrawBoxes(detections, img, depth):
    for detection in detections:
        ...
        pt1 = (xmin, ymin)
        pt2 = (xmax, ymax)
        x, y, z = get_object_depth(depth, detection[2])
        distance = math.sqrt(x * x + y * y + z * z)
        distance = "{:.2f}".format(distance)
        cv2.rectangle(img, pt1, pt2, (0, 255, 0), 1)
        PData = "DP: "+ distance +  " C: (" + str((pt1[0] + pt2[0])/2) + ","+ str((pt1[1] + pt2[1])/2) + ") D: " + str(round((423.94/(pt2[1] - pt1[1])+0.2098),4)) + " H: " + str(round(((pt1[0] + pt2[0])/2)*0.0859 -55,1))
        cv2.putText(img,
                    detection[0] +
                    " [" + str(round(detection[1] * 100, 2)) + "]" + PData,
...
def yolov3():
    ...
    thresh = 0.25
    darknet_path=""
    config_path = "panda/yolov3-tiny-panda.cfg"
    weight_path = "panda/yolov3-tiny-panda_final.weights"
    meta_path = "panda/obj.data"
    svo_path = "HD1080.svo"
    zed_id = 0
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            image = cvDrawBoxes(detections, image, depth)
